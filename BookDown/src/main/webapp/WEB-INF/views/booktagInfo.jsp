...
<header id="header" class="header" style="background-color: #58B4AB;">
    ...
    <div class="navbar" id="nav-header" style="border-bottom: 4px solid #58B4AB ;">
        ...
        <ul class="nav">
            ...
            <li class="menu-item menu-item-type-custom menu-item-object-custom current-menu-item current_page_item menu-item-has-children menu-item-1228"
                id="menu-item-1228">
    ...
    <div class="content-wrap">
        <div class="content">
            <header class="archive-header">
                <h1>标签:${booktag.btName}</h1>
            </header>
            <div class="pagewrapper" style="padding-top:.1px">
                <div class="cardlist" role="main">
                    <c:choose>
                        <c:when test="${not empty bookPage.lists}">
                            <c:forEach items="${bookPage.lists}" var="item">
            ...
            <div class="pagination" style="padding-bottom: 10px">
                <ul>
                    <li><a href="/tag?pn=1&tagid=${booktag.btId}">首页</a></li>
                    <c:choose>
                        <c:when test="${bookPage.currPage !=1}">
                            <li class="prev-page"><a href='/tag?pn=${bookPage.currPage-1}&tagid=${booktag.btId}'>上一页</a>
                            </li>
                        </c:when>
                    </c:choose>

                    <c:choose>
                        <c:when test="${bookPage.totalPage <=6}">
                            <%--1.总页数不足6页--%>
                            <c:set var="begin" value="1"/>
                            <c:set var="end" value="${bookPage.totalPage}"/>
                        </c:when>
                        <c:otherwise>
                            <%--2.通过公式设置begin=当前页-3，end=当前页+3--%>
                            <c:set var="begin" value="${bookPage.currPage-3}"/>
                            <c:set var="end" value="${bookPage.currPage+3}"/>
                            ...
                            <c:if test="${end >bookPage.totalPage}">
                                <c:set var="begin" value="${bookPage.totalPage-5}"/>
                                <c:set var="end" value="${bookPage.totalPage}"/>
                            </c:if>
                        </c:otherwise>
                    </c:choose>
                    ...
                    <c:forEach begin="${begin}" end="${end}" var="i">
                        <c:choose>
                            <c:when test="${i eq bookPage.currPage}">
                                <li class="active"><span>${i}</span></li>
                            </c:when>
                            <c:otherwise>
                                <li><a href="/tag?pn=${i}&tagid=${booktag.btId}">${i}</a></li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <%-- 显示点点--%>
                    ...
                    <%--下一页--%>
                    <c:choose>
                        <c:when test="${bookPage.currPage !=bookPage.totalPage}">
                            <li class="next-page"><a href="/tag?pn=${bookPage.currPage+1}&tagid=${booktag.btId}">下一页</a>
                            </li>
                        </c:when>
                    </c:choose>
                    <%--末页--%>
                    <li><a href="/tag?pn=${bookPage.totalPage}&tagid=${booktag.btId}">末页</a></li>
                </ul>
            </div>
        </div>
